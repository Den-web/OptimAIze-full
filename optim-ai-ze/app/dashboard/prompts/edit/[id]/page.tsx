...
export default function EditPromptPage() {
  ...
  useEffect(() => {
    if (params.id) {
      const existingPrompt = getPrompt(params.id as string)
      if (existingPrompt) {
        setPrompt({
        ...
        setSelectedRules(existingPrompt.ruleIds || [])
      } else {
        router.push("/dashboard/prompts")
      }
    }
  }, [params.id, getPrompt, router])
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-muted-foreground">Update your AI prompt template.</p>
      </div>
      <Card>
        <CardHeader>
